# vL.d3vops — Portfólio de Vitor Ivo Rogério Barbosa Leite

> Data Scientist Jr · Big Data Analytics · Data Mining

Site de portfólio profissional construído em **HTML puro, CSS e JavaScript** — sem frameworks, sem dependências, zero build step.

🔗 **Live:** [vlbarbosa.vercel.app]([https://vlbarbosa.vercel.app](https://vl-d3vops.vercel.app))
📧 **Contato:** vitoor.projects@outlook.com  
🐙 **GitHub:** [github.com/vlbarbosa](https://github.com/vlbarbosa)  
💼 **LinkedIn:** [linkedin.com/in/vlbarbosa](https://linkedin.com/in/vlbarbosa)

---

## ✨ Funcionalidades

- Cursor personalizado com efeito lerp suavizado
- Efeito typewriter com múltiplas frases
- Partículas interativas em canvas (constelação)
- Animações reveal on scroll (IntersectionObserver)
- Barras de habilidade animadas
- Contadores numéricos animados
- Parallax no hero
- Barra de progresso de leitura
- Micro-animação glitch nos cards
- Easter egg Konami Code (↑↑↓↓←→←→BA)
- 100% responsivo para mobile

---

## 📁 Estrutura

```
portfolio/
├── index.html      ← arquivo principal (tudo em um único arquivo)
├── vercel.json     ← configuração de deploy no Vercel
└── README.md       ← este arquivo
```

---
## 🛠️ Tecnologias

| Tecnologia | Uso |
|---|---|
| HTML5 | Estrutura semântica |
| CSS3 | Variáveis, animações, grid, flex |
| JavaScript ES6+ | Canvas, IntersectionObserver, animações |
| Google Fonts | Bebas Neue + DM Mono + DM Sans |

---

*Construído com HTML puro e paixão por dados 🧡*
